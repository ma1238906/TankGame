...
public class RoomListPanel : BasePanel {
	...
	public override void OnShow(params object[] args) {
		...
		headImage = skin.transform.Find("InfoPanel/HeadImage").GetComponent<Image>();
		...
		//创建坦克模型
		GameObject tankSkin = ResManager.LoadPrefab("TankShow");
		tankObj = (GameObject)Instantiate(tankSkin, tankCamera.transform);
		tankObj.transform.localPosition = new Vector3(0, -1.5f, 5);
	...
	public void OnMsgGetAchieve (MsgBase msgBase) {
		...
		//显示玩家信息
		GameMain.player = new PlayerData()
		{
			nickname = msg.nickName,
			headName = msg.headName,
			win = msg.win,
			lost = msg.lost,
		};
		idText.text = msg.nickName;
		scoreText.text = msg.win + "胜 " + msg.lost + "负";
		if(!string.IsNullOrEmpty(msg.headName))
        {
			headImage.sprite = ResManager.LoadAsset<Sprite>("UI/"+msg.headName);
		}
	}
	...
	public void GenerateRoom(RoomInfo roomInfo){
		...
		Text mapText = trans.Find("MapText").GetComponent<Text>();
	...
	public void OnCreateClick() {
		PanelManager.Open<SelectMapPanel>();
	}

	//收到新建房间协议---功能转移到selectMapPanel了
	...
	public void Update(){
		//更新坦克视图
		tankObj.transform.Rotate(0, Time.deltaTime, 0);
	}
}
