...
public class Room {
	...
	public MsgBase ToMsg(){
		...
		foreach(string id in playerIds.Keys){
			...
			playerInfo.nickname = player.data.nickName;
	...
	private void ResetPlayers(){
		...
		//地图加载 true:地图加载完成 false:地图未加载完成
		string[] ids = playerIds.Keys.ToArray();
		for(int i=0;i<ids.Length;i++)
        {
			playerIds[ids[i]] = false;
        }
	...
	//玩家地图加载完成
	public void PlayerLoadComplete(Player player)
    {
		Console.WriteLine(player.id+"加载完成");
		if(playerIds.ContainsKey(player.id))
        {
