...
public partial class MsgHandler {
	...
	public static void MsgGetAchieve(ClientState c, MsgBase msgBase){
		...
		msg.nickName = player.data.nickName;
		msg.headName = player.data.headName;
	...
	public static void MsgEnterRoom(ClientState c, MsgBase msgBase){
		...
		//返回协议	
		msg.roomID = "room" + room.id;//为了加入语音房间
		msg.result = 0;
		player.Send(msg);
	}
	...
	public static void MsgStartBattle(ClientState c, MsgBase msgBase){
		...
		//成功
		msg.result = 0;
		msg.map = room.mapName;
		room.Broadcast(msg);
		//player.Send(msg);
	}

	//地图加载完成消息
	public static void MsgMapReady(ClientState c, MsgBase msgBase)
    {
		MsgMapReady msg = (MsgMapReady)msgBase;
		Player player = c.player;
		if (player == null) return;
		//room
		Room room = RoomManager.GetRoom(player.roomId);
		...
		room.PlayerLoadComplete(player);
	}
